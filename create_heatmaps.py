...
from utils.model import SupConResNet
import numpy as np
...
if __name__ == '__main__':
    ...
    def_filter_params = {'a_t':1, 'a_h': 2, 'max_n_holes':8}
 ...
 #  feature_extractor.load_state_dict(torch.load('ResNetModels/simclr.pt'))
 #  feature_extractor = feature_extractor.encoder.eval()
    ...
    for i in range(len(process_stack)):
        ...
        if os.path.isfile(os.path.join(p_slide_save_dir, heatmap_save_name)):
            ...
            if heatmap_args.save_ext == 'jpg':
                heatmap.save(os.path.join(p_slide_save_dir, heatmap_save_name), quality=70)
            else:
