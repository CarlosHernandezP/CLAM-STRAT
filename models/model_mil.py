...
class MIL_fc(nn.Module):
    def __init__(self, gate = True, size_arg = "small", dropout = False, n_classes = 2, top_k=1, feat_type='simclr'
                 ,dropout_value=0):
...
class MIL_fc_mc(nn.Module):
    def __init__(self, gate = True, size_arg = "small", 
                 dropout = False, n_classes = 2, top_k=1, dropout_value=0):
        super(MIL_fc_mc, self).__init__()
        assert n_classes > 2
        self.size_dict = {"small": [2048, 512]}
        size = self.size_dict[size_arg]
        fc = [nn.Linear(size[0], size[1]), nn.ReLU()]
        if dropout:
            fc.append(nn.Dropout(0.25))
